...
export const Route = createFileRoute("/$")({
  head: () => ({
    meta: [
      { title: "Page not found | Zariya House" },
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-[1100px] place-items-center px-6 py-24 text-center">
        <div>
          ...
          <div className="mt-16 grid gap-6 sm:grid-cols-3">
            {[
              { label: "Bridal Edit", to: "/bridal" as const },
              { label: "Signature Rings", to: "/category/rings" as const },
              { label: "Search Designs", to: "/search" as const },
            ].map((l) => (
              <Link
                key={l.to}
                to={l.to}
