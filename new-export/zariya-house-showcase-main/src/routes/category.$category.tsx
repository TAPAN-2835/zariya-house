...
export const Route = createFileRoute("/category/$category")({
  head: ({ params }) => {
    const c = BROWSE_CATEGORIES.find((x) => x.slug === params.category);
    const title = c?.title ?? "Category";
    return {
      meta: [
        { title: `${title} | Zariya House` },
        { name: "description", content: `Explore ${title.toLowerCase()} designs at Zariya House — curated, editorial, made for every story.` },
        { property: "og:title", content: `${title} | Zariya House` },
        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {BROWSE_CATEGORIES.filter((c) => c.slug !== category).slice(0, 4).map((c) => (
              <Link key={c.slug} to="/category/$category" params={{ category: c.slug }} className="group block">
